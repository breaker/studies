...
void Application::Update(float deltaTime) {
    try {
        ...
        if (KeyDown(VK_RIGHT))
            m_angle += deltaTime;
        if (m_angle >= 2 * D3DX_PI || m_angle <= -2 * D3DX_PI)
            m_angle = 0.0f;
...
void Application::Render() {
    if (!m_deviceLost) {
        try {
            ...
            D3DXMatrixLookAtLH(&view, &D3DXVECTOR3(cos(m_angle) * 2.0f, m_height, sin(m_angle) * 2.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f));
