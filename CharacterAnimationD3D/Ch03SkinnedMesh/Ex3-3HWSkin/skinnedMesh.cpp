...
HRESULT BoneHierarchyLoader::CreateMeshContainer(LPCSTR Name,
    ...
    if (pSkinInfo != NULL) {
        ...
        // 把普通 vertex 转换为 index blended vertex, 后者包含 weights, boneIndices
        // 因此, shader 中的 VS_INPUT_SKIN 也多了这两个成员. 见书 P62
    ...
    if (bone->pMeshContainer != NULL) {
        ...
        if (boneMesh->pSkinInfo != NULL) {
            ...
            for (int i = 0; i < (int) boneMesh->NumAttributeGroups; i++) {
                ...
                g_pDevice->SetTexture(0, boneMesh->textures[mtrlIndex]);    // 固定管线设置纹理

                g_pEffect->SetTexture("texDiffuse", boneMesh->textures[mtrlIndex]); // shader 设置纹理
