...
CMirrorShadowApp MirrorShadowApp;   // 应用程序实例

////////////////////////////////////////////////////////////////////////////////
// 应用程序类 CMirrorShadowApp
////////////////////////////////////////////////////////////////////////////////

BOOL CMirrorShadowApp::InitInstance()
{
...
int CMirrorShadowApp::Exit(int exitCode)
{
...
void CMirrorShadowApp::InitLocale()
{
...
BOOL CMirrorShadowApp::InitModulePath(DWORD size)
{
...
BOOL CMirrorShadowApp::InitWorkDir(const _TCHAR* dir)
{
...
CString CMirrorShadowApp::GetModuleDir() const
{
    int i = m_ModulePath.ReverseFind(_T('\\'));
    return (i == -1 ? _T("") : m_ModulePath.Left(i + 1));   // 没找到 '\' 时, 返回 ""
}

int CMirrorShadowApp::Run()
{
    HWND hwnd = m_pMainWnd->GetSafeHwnd();
    BOOL windowed = TRUE;
    auto_ptr<RenderMirrorShadow> render(new RenderMirrorShadow());
    if (!render->Init(WIN_WIDTH, WIN_HEIGHT, hwnd, windowed, D3DDEVTYPE_HAL))
        return Exit(-1);

    auto_ptr<MirrorShadowInput> input(new MirrorShadowInput());
