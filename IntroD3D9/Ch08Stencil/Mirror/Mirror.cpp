...
int CMirrorApp::Exit(int exitCode)
{
...
BOOL CMirrorApp::InitModulePath(DWORD size)
{
...
BOOL CMirrorApp::InitWorkDir(const _TCHAR* dir)
{
...
CString CMirrorApp::GetModuleDir() const
{
...
int CMirrorApp::Run()
{
    ...
    BOOL windowed = TRUE;
    auto_ptr<RenderMirror> render(new RenderMirror());
    if (!render->Init(WIN_WIDTH, WIN_HEIGHT, hwnd, windowed, D3DDEVTYPE_HAL))
        return Exit(-1);

    auto_ptr<MirrorInput> input(new MirrorInput());
