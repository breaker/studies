...
BOOL Main::PeekMessage(__out int* exitCode, HWND hwnd, IdleFunc idleFunc, PreTransMessageFunc preTransFunc)
{
...
BOOL Main::GetMessage(__out int* exitCode, HWND hwnd, IdleFunc idleFunc, PreTransMessageFunc preTransFunc)
{
    _ASSERTE(exitCode != NULL);
    _ASSERTE(idleFunc != NULL);
    _ASSERTE(preTransFunc != NULL);
    ...
    MSG msg;
    DWORD ret = ::GetMessage(&msg, hwnd, 0, 0);
    if (ret == 0 || ret == -1) {
        *exitCode = (int) msg.wParam;
        return FALSE;
    }
