...
LPCTSTR RenderD3DFont::CRATE_TEXTURE_FILE = _T("crate.jpg");

BOOL RenderD3DFont::Init(UINT width, UINT height, HWND hwnd, BOOL windowed, D3DDEVTYPE devType)
{
    ...
    m_Font.reset(new CD3DFont(_T("Times New Roman"), 16, 0));
    m_Font->InitDeviceObjects(m_D3DDev);
    m_Font->RestoreDeviceObjects();

    return TRUE;
}

RenderD3DFont::~RenderD3DFont()
{
    m_Font->InvalidateDeviceObjects();
    m_Font->DeleteDeviceObjects();
}

void RenderD3DFont::Draw(float timeDelta)
{
    ...
    m_Font->DrawText(10, 10, D3DCOLOR_ARGB(0xff, 0, 0x80, 0), m_FPS.CalcFPSStr(timeDelta));
...
void D3DFontInput::ProcessInput(float timeDelta)
{
    RenderD3DFont* render = (RenderD3DFont*) m_Main->GetRender();
