...
CTexCubeApp  g_TexCubeApp;    // 应用程序实例

////////////////////////////////////////////////////////////////////////////////
// 应用程序类 CTexCubeApp
////////////////////////////////////////////////////////////////////////////////

BOOL CTexCubeApp::InitInstance()
{
...
BOOL CTexCubeApp::InitModulePath(DWORD size)
{
...
BOOL CTexCubeApp::InitWorkDir(const _TCHAR* dir)
{
...
CString CTexCubeApp::GetModuleDir() const
{
    int i = m_ModulePath.ReverseFind(_T('\\'));
    return (i == -1 ? _T("") : m_ModulePath.Left(i + 1));   // 没找到 '\' 时, 返回 ""
}

void CTexCubeApp::SetPumpMsgType(PUMP_MSG_TYPE type)
{
...
BOOL CTexCubeApp::GetMsg()
{
    MSG msg;
    DWORD ret = ::GetMessage(&msg, NULL, 0, 0);
    if (ret == 0 || ret == -1) {
        g_TexCubeApp.m_ExitCode = (int) msg.wParam;
        return FALSE;
    }

    // 将按键消息转换为 WM_COMMAND 消息 (快捷键消息转换), 并同步发送至目标窗口
    if (!g_TexCubeApp.PreTranslateMessage(&msg)) {
    ...
    if (!::PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE)) {
        LONG cnt = 0;
        while (g_TexCubeApp.OnIdle(cnt++));
    }
    return TRUE;
}

BOOL CTexCubeApp::PeekMsg()
{
    MSG msg;
...
{
    try {
        m_GameRun.reset(new SGLGameRun(new RenderTexCube(DEF_WIDTH, DEF_HEIGHT, m_pMainWnd->GetSafeHwnd(), TRUE, D3DDEVTYPE_HAL)));
    }
    catch (SGLExcept& e) {
