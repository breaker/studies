...
CAddressModeApp  g_AddressModeApp;    // 应用程序实例

////////////////////////////////////////////////////////////////////////////////
// 应用程序类 CAddressModeApp
////////////////////////////////////////////////////////////////////////////////

BOOL CAddressModeApp::InitInstance()
{
...
int CAddressModeApp::ExitInstance()
{
    // NOTE:
    // 这里不用加检查内存泄漏的语句, Debug 版的 MFC 程序会自动在程序退出时 (WinMain 结束) 调用 _CrtDumpMemoryLeaks
    // 否则这里会误报: 如果类 SP 和 RAII 型成员没有手工释放, 也会报告泄露, 但实际上并不算是泄露, 典型的如 CWinApp 中的 String, auto_ptr 成员
    // _CrtDumpMemoryLeaks();
...
void CAddressModeApp::InitLocale()
{
...
BOOL CAddressModeApp::InitModulePath(DWORD size)
{
...
BOOL CAddressModeApp::InitWorkDir(const _TCHAR* dir)
{
...
CString CAddressModeApp::GetModuleDir() const
{
    int i = m_ModulePath.ReverseFind(_T('\\'));
    return (i == -1 ? _T("") : m_ModulePath.Left(i + 1));   // 没找到 '\' 时, 返回 ""
}

void CAddressModeApp::SetPumpMsgType(PUMP_MSG_TYPE type)
{
...
BOOL CAddressModeApp::GetMsg()
{
    MSG msg;
    DWORD ret = ::GetMessage(&msg, NULL, 0, 0);
    if (ret == 0 || ret == -1) {
        g_AddressModeApp.m_ExitCode = (int) msg.wParam;
        return FALSE;
    }

    // 将按键消息转换为 WM_COMMAND 消息 (快捷键消息转换), 并同步发送至目标窗口
    if (!g_AddressModeApp.PreTranslateMessage(&msg)) {
    ...
    if (!::PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE)) {
        LONG cnt = 0;
        while (g_AddressModeApp.OnIdle(cnt++));
    }
    return TRUE;
}

BOOL CAddressModeApp::PeekMsg()
{
    MSG msg;
...
int CAddressModeApp::Run()
{
    try {
        m_GameRun.reset(new SGLGameRun(new RenderAddressMode(DEF_WIDTH, DEF_HEIGHT, m_pMainWnd->GetSafeHwnd(), TRUE, D3DDEVTYPE_HAL)));
    }
    catch (SGLExcept& e) {
