...
BOOL CTeapotApp::InitModulePath(DWORD size)
{
...
CString CTeapotApp::GetModuleDir() const
{
    int i = m_ModulePath.ReverseFind(_T('\\'));
    return (i == -1 ? _T("") : m_ModulePath.Left(i + 1));   // 没找到 '\' 时, 返回 ""
}

void CTeapotApp::SetPumpMsgType(PUMP_MSG_TYPE type)
{
...
BOOL CTeapotApp::GetMsg()
{
    MSG msg;
    DWORD ret = ::GetMessage(&msg, NULL, 0, 0);
    if (ret == 0 || ret == -1) {
        g_TeapotApp.m_ExitCode = (int) msg.wParam;
        return FALSE;
    }

    // 将按键消息转换为 WM_COMMAND 消息 (快捷键消息转换), 并同步发送至目标窗口
    if (!g_TeapotApp.PreTranslateMessage(&msg)) {
    ...
    if (!::PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE)) {
        LONG cnt = 0;
        while (g_TeapotApp.OnIdle(cnt++));
    }
    return TRUE;
}

BOOL CTeapotApp::PeekMsg()
{
    MSG msg;
