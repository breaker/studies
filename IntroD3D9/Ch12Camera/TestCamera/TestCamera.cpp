...
CTestCameraApp TestCameraApp; // 应用程序实例
...
int CTestCameraApp::Exit(int exitCode)
{
...
void CTestCameraApp::InitLocale()
{
...
BOOL CTestCameraApp::InitModulePath(DWORD size)
{
...
BOOL CTestCameraApp::InitWorkDir(const _TCHAR* dir)
{
...
CString CTestCameraApp::GetModuleDir() const
{
    int i = m_ModulePath.ReverseFind(_T('\\'));
    return (i == -1 ? _T("") : m_ModulePath.Left(i + 1));   // 没找到 '\' 时, 返回 ""
}

int CTestCameraApp::Run()
{
    HWND hwnd = m_pMainWnd->GetSafeHwnd();
    BOOL windowed = TRUE;
    auto_ptr<RenderTestCamera> render(new RenderTestCamera());
    ...
    auto_ptr<TestCameraInput> input(new TestCameraInput());
