...
HRESULT Render::Init(UINT width, UINT height, HWND hwnd, BOOL windowed, D3DDEVTYPE devType, D3DFORMAT bkBufFmt, D3DMULTISAMPLE_TYPE sampleType, D3DFORMAT depthStencilFmt) {
    // 创建 IDirect3D9 接口
    CComPtr<IDirect3D9> d3d9 = Direct3DCreate9(D3D_SDK_VERSION);
    SGL_TRUE_DO(d3d9.p == NULL, SGL_MRPT(_CRT_ERROR, "Direct3DCreate9 failed"); return S_FALSE);

    // 检测硬件顶点处理
    D3DCAPS9 caps;
    HRESULT hr = d3d9->GetDeviceCaps(D3DADAPTER_DEFAULT, devType, &caps);
    SGL_FAILED_DO(hr, SGL_MRPTDX(_CRT_ERROR, "IDirect3D9::GetDeviceCaps", hr); return hr);
    ...
    if (FAILED(hr)) {
        ...
        hr = d3d9->CreateDevice(D3DADAPTER_DEFAULT, devType, hwnd, devFlag, &d3dpp, &d3ddev);
        SGL_FAILED_DO(hr, SGL_MRPTDX(_CRT_ERROR, "IDirect3D9::CreateDevice", hr); return hr);
    }
    m_D3DDev.Attach(d3ddev);
    m_hWnd = hwnd;

    m_FPS.Init(32, _T("FPS: "));
