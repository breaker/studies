...
CInitD3D1App    g_InitD3D1App;  // 应用程序实例

////////////////////////////////////////////////////////////////////////////////
// 应用程序类 CInitD3D1App
////////////////////////////////////////////////////////////////////////////////

BOOL CInitD3D1App::InitInstance()
{
...
BOOL CInitD3D1App::InitModulePath(DWORD size)
{
...
BOOL CInitD3D1App::InitWorkDir(const _TCHAR* dir)
{
...
CString CInitD3D1App::GetModuleDir() const
{
    int i = m_ModulePath.ReverseFind(_T('\\'));
    return (i == -1 ? _T("") : m_ModulePath.Left(i + 1));   // 没找到 '\' 时, 返回 ""
}

void CInitD3D1App::SetPumpMsgType(PUMP_MSG_TYPE type)
{
...
BOOL CInitD3D1App::GetMsg()
{
    MSG msg;
    DWORD ret = ::GetMessage(&msg, NULL, 0, 0);
    if (ret == 0 || ret == -1) {
        g_InitD3D1App.m_ExitCode = (int) msg.wParam;
        return FALSE;
    }

    // 将按键消息转换为 WM_COMMAND 消息 (快捷键消息转换), 并同步发送至目标窗口
    if (!g_InitD3D1App.PreTranslateMessage(&msg)) {
    ...
    if (!::PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE)) {
        LONG cnt = 0;
        while (g_InitD3D1App.OnIdle(cnt++));
    }
    return TRUE;
}

BOOL CInitD3D1App::PeekMsg()
{
    MSG msg;
...
int CInitD3D1App::Run()
{
    try {
        m_GameRun.reset(new SGLGameRun(new ClearBk(DEF_WIDTH, DEF_HEIGHT, m_pMainWnd->GetSafeHwnd(), TRUE, D3DDEVTYPE_HAL)));
    }
    catch (SGLExcept& e) {
