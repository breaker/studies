...
CSpotLightApp  g_SpotLightApp;    // 应用程序实例

////////////////////////////////////////////////////////////////////////////////
// 应用程序类 CSpotLightApp
////////////////////////////////////////////////////////////////////////////////

BOOL CSpotLightApp::InitInstance()
{
...
int CSpotLightApp::ExitInstance()
{
    // NOTE:
...
BOOL CSpotLightApp::InitModulePath(DWORD size)
{
...
BOOL CSpotLightApp::InitWorkDir(const _TCHAR* dir)
{
...
CString CSpotLightApp::GetModuleDir() const
{
    int i = m_ModulePath.ReverseFind(_T('\\'));
    return (i == -1 ? _T("") : m_ModulePath.Left(i + 1));   // 没找到 '\' 时, 返回 ""
}

void CSpotLightApp::SetPumpMsgType(PUMP_MSG_TYPE type)
{
...
BOOL CSpotLightApp::GetMsg()
{
    MSG msg;
    DWORD ret = ::GetMessage(&msg, NULL, 0, 0);
    if (ret == 0 || ret == -1) {
        g_SpotLightApp.m_ExitCode = (int) msg.wParam;
        return FALSE;
    }

    // 将按键消息转换为 WM_COMMAND 消息 (快捷键消息转换), 并同步发送至目标窗口
    if (!g_SpotLightApp.PreTranslateMessage(&msg)) {
    ...
    if (!::PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE)) {
        LONG cnt = 0;
        while (g_SpotLightApp.OnIdle(cnt++));
    }
    return TRUE;
}

BOOL CSpotLightApp::PeekMsg()
{
    MSG msg;
...
int CSpotLightApp::Run()
{
    try {
        m_GameRun.reset(new SGLGameRun(new RenderSpotLight(DEF_WIDTH, DEF_HEIGHT, m_pMainWnd->GetSafeHwnd(), TRUE, D3DDEVTYPE_HAL)));
    }
    catch (SGLExcept& e) {
