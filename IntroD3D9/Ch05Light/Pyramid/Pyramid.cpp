...
CPyramidApp    g_PyramidApp;  // 应用程序实例
...
BOOL CPyramidApp::InitModulePath(DWORD size)
{
...
BOOL CPyramidApp::InitWorkDir(const _TCHAR* dir)
{
...
CString CPyramidApp::GetModuleDir() const
{
    int i = m_ModulePath.ReverseFind(_T('\\'));
    return (i == -1 ? _T("") : m_ModulePath.Left(i + 1));   // 没找到 '\' 时, 返回 ""
}

void CPyramidApp::SetPumpMsgType(PUMP_MSG_TYPE type)
{
...
BOOL CPyramidApp::GetMsg()
{
    MSG msg;
    DWORD ret = ::GetMessage(&msg, NULL, 0, 0);
    if (ret == 0 || ret == -1) {
        g_PyramidApp.m_ExitCode = (int) msg.wParam;
        return FALSE;
    }

    // 将按键消息转换为 WM_COMMAND 消息 (快捷键消息转换), 并同步发送至目标窗口
    if (!g_PyramidApp.PreTranslateMessage(&msg)) {
    ...
    if (!::PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE)) {
        LONG cnt = 0;
        while (g_PyramidApp.OnIdle(cnt++));
    }
    return TRUE;
}

BOOL CPyramidApp::PeekMsg()
{
    MSG msg;
...
{
    try {
        m_GameRun.reset(new SGLGameRun(new RenderPyramid(DEF_WIDTH, DEF_HEIGHT, m_pMainWnd->GetSafeHwnd(), TRUE, D3DDEVTYPE_HAL)));
    }
    catch (SGLExcept& e) {
