...
CPointLightApp  g_PointLightApp;    // 应用程序实例

////////////////////////////////////////////////////////////////////////////////
// 应用程序类 CPointLightApp
////////////////////////////////////////////////////////////////////////////////

BOOL CPointLightApp::InitInstance()
{
...
int CPointLightApp::ExitInstance()
{
    // NOTE:
...
void CPointLightApp::InitLocale()
{
...
BOOL CPointLightApp::InitModulePath(DWORD size)
{
...
BOOL CPointLightApp::InitWorkDir(const _TCHAR* dir)
{
...
CString CPointLightApp::GetModuleDir() const
{
    int i = m_ModulePath.ReverseFind(_T('\\'));
    return (i == -1 ? _T("") : m_ModulePath.Left(i + 1));   // 没找到 '\' 时, 返回 ""
}

void CPointLightApp::SetPumpMsgType(PUMP_MSG_TYPE type)
{
...
BOOL CPointLightApp::GetMsg()
{
    MSG msg;
    DWORD ret = ::GetMessage(&msg, NULL, 0, 0);
    if (ret == 0 || ret == -1) {
        g_PointLightApp.m_ExitCode = (int) msg.wParam;
        return FALSE;
    }

    // 将按键消息转换为 WM_COMMAND 消息 (快捷键消息转换), 并同步发送至目标窗口
    if (!g_PointLightApp.PreTranslateMessage(&msg)) {
    ...
    if (!::PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE)) {
        LONG cnt = 0;
        while (g_PointLightApp.OnIdle(cnt++));
    }
    return TRUE;
}

BOOL CPointLightApp::PeekMsg()
{
    MSG msg;
...
int CPointLightApp::Run()
{
    try {
        m_GameRun.reset(new SGLGameRun(new RenderPointLight(DEF_WIDTH, DEF_HEIGHT, m_pMainWnd->GetSafeHwnd(), TRUE, D3DDEVTYPE_HAL)));
    }
    catch (SGLExcept& e) {
