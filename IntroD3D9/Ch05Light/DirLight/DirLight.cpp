...
CDirLightApp    g_DirLightApp;  // 应用程序实例

////////////////////////////////////////////////////////////////////////////////
// 应用程序类 CDirLightApp
////////////////////////////////////////////////////////////////////////////////

BOOL CDirLightApp::InitInstance()
{
...
BOOL CDirLightApp::InitModulePath(DWORD size)
{
...
BOOL CDirLightApp::InitWorkDir(const _TCHAR* dir)
{
...
CString CDirLightApp::GetModuleDir() const
{
    int i = m_ModulePath.ReverseFind(_T('\\'));
    return (i == -1 ? _T("") : m_ModulePath.Left(i + 1));   // 没找到 '\' 时, 返回 ""
}

void CDirLightApp::SetPumpMsgType(PUMP_MSG_TYPE type)
{
...
BOOL CDirLightApp::GetMsg()
{
    MSG msg;
    DWORD ret = ::GetMessage(&msg, NULL, 0, 0);
    if (ret == 0 || ret == -1) {
        g_DirLightApp.m_ExitCode = (int) msg.wParam;
        return FALSE;
    }

    // 将按键消息转换为 WM_COMMAND 消息 (快捷键消息转换), 并同步发送至目标窗口
    if (!g_DirLightApp.PreTranslateMessage(&msg)) {
    ...
    if (!::PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE)) {
        LONG cnt = 0;
        while (g_DirLightApp.OnIdle(cnt++));
    }
    return TRUE;
}

BOOL CDirLightApp::PeekMsg()
{
    MSG msg;
...
int CDirLightApp::Run()
{
    try {
        m_GameRun.reset(new SGLGameRun(new RenderDirLight(DEF_WIDTH, DEF_HEIGHT, m_pMainWnd->GetSafeHwnd(), TRUE, D3DDEVTYPE_HAL)));
    }
    catch (SGLExcept& e) {
