...
{
    ((CColorTriangleApp*) AfxGetApp())->GetGameRun()->stop();
    CFrameWnd::OnClose();
}
...
void CMainFrame::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
    ...
    if (nState == WA_INACTIVE || bMinimized) {
        m_GameRun->stop();
        g_ColorTriangleApp.SetPumpMsgType(CColorTriangleApp::PUMP_GET_MSG);
    }
    else {
        m_GameRun->resume();
        g_ColorTriangleApp.SetPumpMsgType(CColorTriangleApp::PUMP_PEEK_MSG);
    }
}
