...
LPCTSTR RenderFirework::MeshDumpFile = _T("MeshDump.txt");
LPCTSTR RenderFirework::TextureFile = _T("flare.bmp");

BOOL RenderFirework::Init(UINT width, UINT height, HWND hwnd, BOOL windowed, D3DDEVTYPE devType) {
    ...
    m_Explosion.reset(new SGL::Firework(origin, 6000));
    if (!m_Explosion->Init(m_D3DDev, TextureFile))
        return FALSE;
...
void RenderFirework::DumpMesh(LPCTSTR fname, ID3DXMesh* mesh) {
    MeshDump dump;
    if (!dump.Init(fname))
        return;
...
void RenderFirework::Draw(float timeDelta) {
    ...
    m_Explosion->Update(timeDelta);

    if (m_Explosion->IsDead())
        m_Explosion->Reset();
...
void FireworkInput::ProcessInput(float timeDelta) {
    RenderFirework* render = (RenderFirework*) m_Main->GetRender();
