...
LPCTSTR RenderLaser::MeshDumpFile = _T("MeshDump.txt");
LPCTSTR RenderLaser::TextureFile = _T("flare_alpha.dds");
...
void RenderLaser::DumpMesh(LPCTSTR fname, ID3DXMesh* mesh) {
    MeshDump dump;
    if (!dump.Init(fname))
        return;
...
void RenderLaser::Draw(float timeDelta) {
...
void LaserInput::ProcessInput(float timeDelta) {
    RenderLaser* render = (RenderLaser*) m_Main->GetRender();
    ...
    const float ONEKEY_DURATION = 0.1f; // 小于这个持续时间都算一次 KEY_DOWN
    SGL::PSystem* gun = render->m_Gun.get();

    // 因为 KEY_DOWN 所用的 GetAsyncKeyState 对击键响应过于灵敏, 即使快速击键都会被 KEY_DOWN 检测出多次, 所以用击键持续时间控制
    // 原示例为了避免这个问题, 在窗口过程中处理 WM_KEYDOWN, 而没放到主循环中
    // 原示例的注释: we use the message system over GetAsyncKeyState because
