...
template <class T>
inline void Delete(__inout T*& p) {
    if (p != NULL) {
        delete p;
        p = NULL;
    }
...
inline float RandomFloat(float lowBound, float highBound) {
...
inline void RandomVector(__out D3DXVECTOR3* out, const D3DXVECTOR3& min, const D3DXVECTOR3& max) {
