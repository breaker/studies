...
int CTransformApp::Exit(int exitCode) {
...
BOOL CTransformApp::InitModulePath(DWORD size) {
...
BOOL CTransformApp::InitWorkDir(const _TCHAR* dir) {
...
CString CTransformApp::GetModuleDir() const {
    int i = m_ModulePath.ReverseFind(_T('\\'));
    return (i == -1 ? _T("") : m_ModulePath.Left(i + 1));   // 没找到 '\' 时, 返回 ""
}

int CTransformApp::Run() {
    HWND hwnd = m_pMainWnd->GetSafeHwnd();
    BOOL windowed = TRUE;
    auto_ptr<RenderTransform> render(new RenderTransform());
    ...
    auto_ptr<TransformInput> input(new TransformInput());
